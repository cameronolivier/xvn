...
//!
//! Provides `anvs set` command for easily updating individual config settings
...
pub fn set_config(setting: Option<String>) -> Result<()> {
    ...
    let config_file = home.join(".anvsrc");
    let mut config = if config_file.exists() {
        ...
        output::warning("No config file found. Please run 'anvs init' first.");
        return Ok(());
    };
...
fn set_use_default(config: &mut Config) -> Result<()> {
    ...
    println!();
    println!("  When enabled, anvs automatically switches to your version manager's");
...
fn save_config(config: &Config, path: &PathBuf) -> Result<()> {
    ...
    let content = format!(
        "# anvs configuration file\n\
         # Generated by: anvs set\n\
         # Last modified: {}\n\
         #\n\
         # To modify this configuration, run: anvs set <setting>\n\
         \n\
         ...
         # When enabled, anvs switches to your version manager's default Node.js version\n\
